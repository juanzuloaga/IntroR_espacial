...
-   Juan Zuloaga.
-   Prof. [Melissa Guzmán](https://www.edslab.org/) (University of Southern California).
...
-   [IntroR_manejar datos](https://juanzuloaga.github.io/IntroR_ManejarDatos/IntroR_ManejarDatos.html)
...
-   `microbenchmark`: Mide el tiempo de procesamiento (parece ser mejor que la función `system.time()` que viene en el paquete base). ([microbenchmark](https://www.rdocumentation.org/packages/microbenchmark/versions/1.4.10/topics/microbenchmark))
...
-   `cood. ref.:` la función asigna por defecto coordenadas geográficas lon/lat, usando WGS 84 (<a href="https://en.wikipedia.org/wiki/World_Geodetic_System" target="_blank">World Geodetic System</a>, por sus siglas en inglés).
...
Por ejemplo en las imágenes Landsat 8 de la (<a href="https://www.earthdata.nasa.gov/learn/backgrounders/remote-sensing" target="_blank">NASA</a>) se muestra la ciudad de Reykjavik en Islandia (July 7, 2019) en donde se pueden apreciar tres resoluciones (30m, 100m y 300m). Es decir desde una resolución fina con más detalle (cada pixel en el terreno es de 30m por 30m) a una más gruesa y borrosa (cada pixel en el terreno es de 300m por 300m).
...
<center>![](https://www.earthdata.nasa.gov/s3fs-public/2022-02/spatial_resolution.jpg){width="80%" height="100%"}</center>
...
<center>![](./Figuras/local_method.png){width="100%" height="100%"}</center>
...
Existe tambien la posibilidad de descargar las [19 bioclimatic variables](https://www.worldclim.org/data/bioclim.html) disponibles utilizando `var` = 'bio'. Por ahora descargemos la temperatura media (tagv):
...
<center>![](./Figuras/zonal_method.png)</center>
...
Note que en nuestros códigos vamos a utilizar el simbolo `%>%`, conocido como `forward-pipe operator` en inglés, que envia el resultado de una expresión a la otra función dentro de un mismo objeto. Este operador`%>%` fue originalmente desarrollado paquete <a href="https://cran.r-project.org/web/packages/magrittr/index.html" target="_blank">`magrittr`</a>.
...
<br> Lo que vamos hacer es copiar el objeto `paises_r` y llamarlo `Colombia`, luego vamos a reclasificar todos los valores diferentes a 43 con NA (sin valores), es decir escoger `Colombia`.
...
```{r, echo=FALSE}
# Verifique cual la ruta y folder en donde R está guardando los archivos temporales, utilizando ésta función
  rasterTmpFile()

# Si quiere cambiar la ruta y folder utilice ésta función
  rasterOptions(tmpdir = "./Temp_para_borrar") #Note que estoy utilizando la ruta relativa para crear un folder en mi proyecto "C:/Talleres_R/IntroR_espacial"
